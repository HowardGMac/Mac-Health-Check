...
This text-only reference documents the key configurable defaults and runtime inventory for Mac Health Check `4.0.0b2`. No diagram is included; use [03-health-check-categories.md](03-health-check-categories.md) for a visual overview.

---

## 4.0.0b2 Runtime Notes

- `operationMode` is documented for the `4.0.0b2` release as `Self Service` by default, with `Silent`, `Debug`, `Development`, and `Test` also supported.
